...
export function InterviewContent({ projectId, interviewId }: InterviewContentProps) {
          ...
          <div className="max-w-4xl mx-auto px-6 py-3">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <Button variant="ghost" size="icon" asChild className="hover:bg-gray-100 -ml-2">
                ...
                <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
                  ...
                  <span className="text-xs text-gray-500 px-1 min-w-[30px] text-center">{currentIndex + 1}/{projectInterviews?.length}</span>
              ...
              <ExportDialog interview={interviewForExport} trigger={<Button variant="outline" size="sm" className="h-8 text-xs border-gray-200 text-gray-600"><Download className="w-3 h-3 mr-1" />Export</Button>} />
            </div>
            <div className="mb-3">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full -ml-2">
              <TabsList className="bg-transparent border-b-0 rounded-none h-9 p-0 gap-1">
                <TabsTrigger value="transcription" className="rounded-md data-[state=active]:bg-[#4CA771] data-[state=active]:text-white px-3 py-1.5 text-sm text-gray-500 hover:text-gray-900 transition-all">
                  <FileSpreadsheet className="w-3.5 h-3.5 mr-1.5" />
                  Transcription
                </TabsTrigger>
                <TabsTrigger value="insights" className="rounded-md data-[state=active]:bg-[#4CA771] data-[state=active]:text-white px-3 py-1.5 text-sm text-gray-500 hover:text-gray-900 transition-all">
                  <Lightbulb className="w-3.5 h-3.5 mr-1.5" />
                  Insights
                  {insights && insights.length > 0 && <span className="ml-1.5 text-xs bg-white/20 px-1 rounded">{insights.length}</span>}
                </TabsTrigger>
                <TabsTrigger value="summary" className="rounded-md data-[state=active]:bg-[#4CA771] data-[state=active]:text-white px-3 py-1.5 text-sm text-gray-500 hover:text-gray-900 transition-all">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Input placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-48 h-9 bg-gray-50 border-gray-200 focus:bg-white focus:border-[#4CA771]" />
                        ...
                        <div className="space-y-4 py-4">
                          <div className="flex gap-2 flex-wrap">
                            {Object.entries(insightTypeConfig).map(([key, config]) => (
                              <button key={key} onClick={() => setNewInsightType(key as Insight['type'])} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${newInsightType === key ? `${config.bg} ${config.color} ring-2 ring-offset-2 ring-current` : 'bg-gray-100 hover:bg-gray-200 text-gray-600'}`}>
                                {config.label}
                              </button>
                            ))}
                          </div>
                          <textarea value={newInsightText} onChange={e => setNewInsightText(e.target.value)} placeholder="Type your insight..." className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg resize-none min-h-[100px] focus:border-[#4CA771] focus:ring-1 focus:ring-[#4CA771]" />
                          <div className="flex justify-end gap-2">
                            <Button variant="outline" onClick={() => setShowAddInsight(false)} className="border-gray-200">Cancel</Button>
                ...
                {!insights || insights.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {filteredInsights.map((insight) => {
                      ...
                      return (
                        <motion.div key={insight._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="group flex items-start gap-4 p-5 bg-white rounded-xl border border-gray-200 hover:border-[#4CA771]/30 hover:shadow-sm transition-all">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <span className="text-xs text-gray-400 font-mono">{Math.floor(insight.timestamp / 60)}:{String(Math.floor(insight.timestamp % 60)).padStart(2, '0')}</span>
                              </div>
                            </div>
                ...
                {interview.summary ? (
                  <>
                    <div className="bg-gray-50 rounded-2xl border border-gray-200 p-6">
                      ...
                      <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-wrap">{interview.summary.executiveSummary}</p>
                    </div>
                      ...
                      <div className="bg-red-50 rounded-2xl border border-red-200 p-6">
                        ...
                        <ul className="space-y-2">
                          {interview.summary.criticalIssues.map((issue: any, i: number) => {
                            ...
                            return <li key={i} className="text-sm p-3 bg-white rounded-lg text-red-600 border border-red-100">{text}</li>;
                          })}
                        </ul>
                      </div>
                    )}
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-slate-50 overflow-hidden">
      {/* Header fixe */}
      <header className="shrink-0 h-auto bg-white border-b border-slate-200 shadow-sm w-full">
        <div className="max-w-4xl mx-auto px-6 py-4">
          ...
          {interview.audioUrl && (
            <div className="mb-4 bg-gradient-to-r from-slate-50 to-white rounded-xl px-4 py-3 border border-slate-200 shadow-sm">
              <AudioPlayer ref={audioPlayerRef} src={interview.audioUrl} onTimeUpdate={setCurrentTime} />
            </div>
          )}
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full -ml-2">
